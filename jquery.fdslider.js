 ...
 (function() {
    ...
    function fdSlider(inp, range, callback, classname, hide, tween, vertical) {
        ...
        this.events = {
            ...
            trackmousewheel: function(e) {
                ...
                if (delta) {
                    ...
                    if (self._vertical)
                        self.handle.style.top = xtmp + "px";
                    else
                        self.handle.style.left = xtmp + "px";
            ...
            onHmousedown: function(e) {
                ...
                if (window.devicePixelRatio || (document.all && !window.opera))
                    try {
                    setTimeout(function() {
                        self.handle.focus();
                    }, 0);
            ...
            onmousedown: function(e) {
                ...
                if (e.pageX)
                    posx = self._vertical ? e.pageY: e.pageX;
                else if (e.clientX)
                    posx = self._vertical ? e.clientY + sTop: e.clientX + sLft;
                ...
                if (posx < 0)
                    posx = 0;
                else if (!self._vertical && posx > self.outerWrapper.offsetWidth - self.handle.offsetWidth)
                    posx = self.outerWrapper.offsetWidth - self.handle.offsetWidth;
                else if (self._vertical && posx > self.outerWrapper.offsetHeight - self.handle.offsetHeight)
                    posx = self.outerWrapper.offsetHeight - self.handle.offsetHeight;
        ...
        this.onTimer = function() {
            ...
            self.updateInput(xtmp);
            if (xtmp != self._posx)
                self._timer = setTimeout(self.onTimer, 200);
            else
                self._kbEnabled = true;
        };
        ...
        this.tweenTo = function(x) {
            ...
            self._frame = 0;
            if (!self._timer)
                self._timer = setTimeout(self.tween, 50);
        };
        ...
        this.tween = function() {
            ...
            self.updateInput(x);
            if (t != d && !self._md)
                self._timer = setTimeout(self.tween, 20);
            else {
        ...
        this.findLabel = function() {
            var label;
            if (self._inp.parentNode && self._inp.parentNode.tagName.toLowerCase() == "label")
                label = self._inp.parentNode;
            else {
            ...
            if (label && !label.id) {
                label.id = inp.id + "_label_" + fdSliderController.uniqueid++;
            };
            return label;
        };
        ...
        this.build = function() {
            if (self._hideInput)
                self._inp.style.display = "none";
            else
                fdSliderController.addEvent(self._inp, 'change', self.events.onchange);
            ...
            self.setAttrNS(self.handle, self.NS_XHTML, "role", "wairole:slider");
            // role:slider
            self.setAttrNS(self.handle, self.NS_STATE, "valuemin", self._min);
            // aaa:valuemin
            self.setAttrNS(self.handle, self.NS_STATE, "valuemax", self._max);
            // aaa:valuemax
            //self.setAttrNS(self.handle, self.NS_STATE, "valuenow", self._inp.value);      // aaa:valuenow
            var lbl = self.findLabel();
            if (lbl) {
                self.setAttrNS(self.handle, self.NS_STATE, 'labelledby', lbl.id);
                // aaa:labelledby
            ...
            if (document.getElementById("fd_slider_describedby")) {
                self.setAttrNS(self.handle, self.NS_STATE, 'describedby', "fd_slider_describedby");
                // aaa:describedby
                };
        ...
        this.destroy = function() {
            try {
                ...
                if (window.addEventListener && !window.devicePixelRatio)
                    window.removeEventListener('DOMMouseScroll', self.events.trackmousewheel, false);
                else {
                    fdSliderController.removeEvent(document, "mousewheel", self.events.trackmousewheel);
                    fdSliderController.removeEvent(window, "mousewheel", self.events.trackmousewheel);
                };
    ...
    fdSliderController = {
        ...
        onload: function(e) {
            for (slider in fdSliderController.sliders) {
                fdSliderController.sliders[slider].resetHandlePosition();
            }
        },
        ...
        construct: function(e) {
            ...
            for (var i = 0, inp; inp = inputs[i]; i++) {
                if ((inp.tagName.toLowerCase() == "input" && inp.type == "text" && inp.className && inp.className.search(regExp_1) != -1) || (inp.tagName.toLowerCase() == "select" && inp.className.search(/fd_slider/) != -1)) {
                    ...
                    if (!inp.id)
                        inp.id == "sldr" + fdSliderController.uniqueid++;
                    // Has the slider already been created?
                    if (document.getElementById("fd-slider-" + inp.id))
                        continue;
